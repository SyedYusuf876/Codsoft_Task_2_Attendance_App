...
public class DbHelper extends SQLiteOpenHelper {
    ...
    private static final String SELECT_STUDENT_TABLE = "SELECT * FROM " + STUDENT_TABLE_NAME + " WHERE " + C_ID + " = ?";
    ...
    private static final String CREATE_STATUS_TABLE =
            "CREATE TABLE " + STATUS_TABLE_NAME + " (" +
                    ...
                    C_ID + " INTEGER NOT NULL, " +
                    DATE_KEY + " TEXT NOT NULL, " + // DATE is stored as TEXT for SQLite compatibility
    ...
    Cursor getStudentTables(long cid) {
        SQLiteDatabase database = this.getReadableDatabase();
    ...
    String getStatus(long sid, String date) {
        ...
        SQLiteDatabase database = this.getReadableDatabase();
        String whereClause = DATE_KEY + "=? AND " + S_ID + "=?";
        Cursor cursor = database.query(STATUS_TABLE_NAME, null, whereClause, new String[]{date, String.valueOf(sid)}, null, null, null);
    ...
    Cursor getDistinctMonths(long cid) {
        SQLiteDatabase database = this.getReadableDatabase();
        String query = "SELECT substr(" + DATE_KEY + ", 4, 7) AS month_year " +
                "FROM " + STATUS_TABLE_NAME + " " +
                "WHERE " + C_ID + " = ? " +
                "GROUP BY month_year";
        return database.rawQuery(query, new String[]{String.valueOf(cid)});
    }
}
